...
uses
  ...
  System.Net.HttpClient, IdHTTP, System.NetEncoding, Vcl.AppEvnts, Vcl.Menus,
  Config, DAO, Global, PUB.Dialogs, PUB.Funcoes, Tipos, rest.HttpClient,
  IdBaseComponent, IdComponent, IdIOHandler, IdIOHandlerSocket,
  IdIOHandlerStack, IdSSL, IdSSLOpenSSL, ssl_openssl, IdTCPConnection,
  IdTCPClient;
...
var
  ...
  IdHTTP1: TIdHTTP;
  LResponse, responsePost: IResponse;
  ...
  retornoApiAlsti, descricao, preco, sku, visivel, cervejaria, tipo, estilo,
    ...
    codigoDesabilitar, jsonprod: string;
...
begin
  ...
  try
    WaitShowMessage('Exportando Lista de Produtos!');
    ...
    begin
      ...
      try
        ...
        begin
          ...
          // Insere categoria
          jsonCategoria := '{"name": "' + categoria + '",' + '"category_id":' +
            codCat + '}';

          LResponse := TRequest.New.BaseURL
            (urlBaseMytapp + '/api/v1/createCategory')
            .AddHeader('token', tokenMytapp).ContentType('application/json')
            .AddBody(jsonCategoria).Post;
          ...
          try
            ...
            if imagemCervejaria <> '' then
              xRequestBody.Add('image_brewery=' + url + imagemCervejaria);

            // Grava registo no mytapp
            response := objHTTP.Post(urlBaseMytapp + '/api/v1/newItemProduct',
              xRequestBody);
...
begin
  ...
  try
    WaitShowMessage('Importando Lista de Clientes!');
...
begin
  ...
  if SpeedButtonOnOff.Caption = 'Ligado' then
  begin
    SpeedButtonOnOff.Caption := 'Desligado';

    Timer.Enabled := False;
  end
  else if SpeedButtonOnOff.Caption = 'Desligado' then
  begin
